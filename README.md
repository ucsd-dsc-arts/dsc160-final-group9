...
- We used a LSTM network to generate music after training the model with roughly 80-min music extracted in the "TikTok Trendy Music Chart" category. Based on the characteristics of LSTM networks that they are good at recognising and encoding long-term patterns, the model predicted the next note or chord based on the previous one. In the last, we generated 500 notes (or chords) for about 2-min long after we provided the first note (or chord). 
