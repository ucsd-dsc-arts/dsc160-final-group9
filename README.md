...
### Audio Generation
<b>classical_piano_composer</b>
- We used a LSTM network to generate music after training the model [train_model.ipynb](https://github.com/ucsd-dsc-arts/dsc160-final-group9/blob/master/code/train_model.ipynb) with roughly 80-min music extracted in the "TikTok Trendy Music Chart" category. The format of the music is MIDI, containing information such as pitch, octave and offset. Based on the characteristics of LSTM networks that they are good at recognising and encoding long-term patterns, the model predicted the next note or chord based on the previous one. [predict.ipynb](https://github.com/ucsd-dsc-arts/dsc160-final-group9/blob/master/code/predict.ipynb) In the last, we generated 500 notes (or chords) for about 2-min long after we provided the first note (or chord). 
- According to the graph of plotted notes (below) from the output melody, we can see that the range of the pitch varies from 40 to 94, which is very broad. Moreover, there are lots of repetitive notes or chords that make some parts of the output melody very unchangeble and boring. Lastly, for the second half of the output melody starting from 60s, the presences of repetitive notes (or chords) increase compared with those in the first half. ![](results/melody_rnn/bokeh_plot_cpt1.png)
- The following two plots are visualized chroma features fitted to our two melodies. The first melody is converted from the output MIDI file from the model with training loss 4.74 at epoch 17, while the second melody is output from the model with training loss 3.47 at the epoch 125. Compared with two graphs, they indicate the increasing variation in notes of a melody as the training epoch increases and training loss decreases. 
![](results/classical_piano_composer/graph.png)
- We train the LSTM network for 200 epoches with batch size of 64. The training loss is indicated in the following graph. ![](results/classical_piano_composer/loss.png)
